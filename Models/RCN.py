...
class CAGATAblationLayer(nn.Module):
    """
    ...
    def __init__(self, in_dim, out_dim, heads=1, dropout=0.0, eps=1e-8,
        ...
        super(CAGATAblationLayer, self).__init__()
        self.in_dim = in_dim
...
class CAGATAblationModel(nn.Module):
    """
    Wrapper for two-layer CA-GAT with ablation support for attention strategies and temperature.
    """
    ...
    def __init__(self, in_dim, hidden_dim, out_dim, heads=1, dropout=0.0,
        ...
        super().__init__()
        self.layer1 = CAGATAblationLayer(
            in_dim=in_dim,
            out_dim=hidden_dim,
            heads=heads,
            dropout=dropout,
        ...
        self.layer2 = CAGATAblationLayer(
...
class RCNAblationLayer(nn.Module):
    """
    Cycle-Aware Graph Attention Layer with flexible bias integration for ablation studies.
    """
    ...
    def __init__(self, in_dim, out_dim, heads=1, dropout=0.0, eps=1e-8,
                 attention_strategy='pre_softmax_bias', temperature=1.0, add_self_loops=False):
        super(RCNAblationLayer, self).__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.heads = heads
        self.eps = eps
...
class RCN_TestModel(nn.Module):
    ...
    def __init__(self, in_dim, hidden_dim, out_dim, heads=1, dropout=0.0,
        ...
        self.out_proj = nn.Linear(hidden_dim * heads, out_dim)
        self.cluster_head = nn.Linear(hidden_dim * heads, out_dim)

    def forward(self, x, edge_index, rnbrw_weights):
        h1, attn1 = self.layer1(x, edge_index, rnbrw_weights)
        h2, attn2 = self.layer2(h1, edge_index, rnbrw_weights)
        S = F.softmax(self.cluster_head(h2), dim=1)
        return S, (attn1, attn2), h2
