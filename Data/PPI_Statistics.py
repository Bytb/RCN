...
#---looking at singleton communities---#
import collections

def count_valid_labels(file_path, unlabeled_token="-1"):
    """
    Count the number of ground truth labels in a .txt file,
    excluding:
      1. Unlabeled nodes (default = -1).
      2. Labels that only correspond to a single node.

    Args:
        file_path (str): Path to the .txt file containing labels.
        unlabeled_token (str|int): Token used for unlabeled nodes (default: "-1").

    Returns:
        int: Number of valid ground truth labels.
    """
    labels = []
    ...
    # --- Filter unlabeled ---
    filtered = [lbl for lbl in labels if lbl != str(unlabeled_token)]

    # --- Count frequencies ---
    counter = collections.Counter(filtered)

    # --- Keep only labels that appear more than once ---
    valid_labels = [lbl for lbl, count in counter.items() if count > 1]

    return len(valid_labels)


if __name__ == "__main__":
    file_path = "/Users/calebfernandes/PycharmProjects/RCN_Test/Data/raw_data/corum_humanComplexes.txt"  # change to your file
    num_labels = count_valid_labels(file_path)
    print(f"Number of valid ground truth labels: {num_labels}")
