...
CONFIG_CSV = os.path.join(os.path.dirname(__file__), "configs", "PPI_configs.csv")

DATASET = "PPI"
K_FIXED = 1995  # fixed number of clusters for k-means (same as your script)
...
def _ensure_dirs():
    out_dir = os.path.join(REPO_DIR, "RCN_Simulations", "logs", "PPI", "partial")
...
def main():
    ...
    try:
        ...
        # --- Load data ---
        x, edge_index, y, edge_weight, G, communities, y_multi, mask, nid2idx, idx2nid = load_ppi_graph(use_onehot=True, device=device)
